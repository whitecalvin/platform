...
const mg_tweet = mongoose.model('tweet', tweetSchema);
...
export async function getAll(pageNo, pageSize) {
    switch (dbType) {
        ...
        case 'MNG':
            return mg_tweet.find().sort({ createdAt: -1 });
    }
}
    ...
    switch (dbType) {
        ...
        case 'MON':
            ...
            return getTweets()
                ...
                .then((tweets) => mapTweets(tweets));
        case 'MNG':
            return mg_tweet.find({ username }).sort({ createdAt: -1 });
    }
}
...
export async function update(id, text) {
    switch (dbType) {
        ...
        case 'MNG':
            // mongoose (ODM)
            return mg_tweet.findByIdAndUpdate(
                id,
                { text },
                { returnOriginal: false }
            );
...
export async function remove(id) {
    switch (dbType) {
        ...
        case 'MNG':
            // mongoose (ODM)
            return mg_tweet.findByIdAndDelete(id);
    }
}
